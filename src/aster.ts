...
export function getAST(sourceFile) {
    emit(sourceFile);

    function emitNodeArray(narr: ts.NodeArray<ts.Node>) : AstNode[] {
        let output = []
        for (var elt of narr) {
            output.push(emit(elt));
        }
        return output
    }

    function emit(val) : AstNode | AstNode [] {
        if (!val)
            return null;
        ...
        return emitArbitraryNode(node);

    }

    function emitArbitraryNode(node: ts.Node) : AstNode {
        let kind = getTypeOfNode(node);
        let childs = []
        for(var key in node) {
            if (key === "parent" ||
                key === "pos" ||
